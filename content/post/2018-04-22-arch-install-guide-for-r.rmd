...
Currently  the best wrapper around `pacman`  is `trizen`.
...
Install (`trizen zsh`) and use it (`zsh`).
...
When calling `trizen <package>` a search in the official repos AND the AUR is done.  This is different to `pacman` which only search in the official repos. Now you know why you need a wrapper :-) 
...
- **Dropbox**: `trizen -S dropbox` (`dropbox-cli` for command line access)  
...
I currently use [Neovim](https://neovim.io/) which is a fork of `vim` for quick editing on the command line. My general purpose IDE is [Visual Studio Code](https://code.visualstudio.com/) and for R development I use `RStudio`.
...
KDE is probably the DE (Desktop environment) that is most customizable. By default it comes with a Windows-like navigation panel. However, you can have a dock by using [latte-dock](https://github.com/psifidotos/Latte-Dock). I used KDE for quite some time and I was satisfied with most of it.
...
[mackup](https://github.com/lra/mackup) is a great tool for this. It syncs a variety of config files and uploads these to a cloud of your choice. Under the hood, the config files will get soft-linked to your cloud which means that updating it on one machine will also trigger a change on all other machines.
...
This Gist stores all of your installed packages. Here's [mine](https://gist.github.com/pat-s/6509f87cf380cbe81631e444079e90e2).
